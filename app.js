...
const TRANSLATIONS = {
  fr: {
    ...
    tester_mode_title: "MODE TESTEUR",
    tester_mode_toggle_label: "Activer le mode testeur",
    tester_mode_warning: "Ta progression est sauvegardée exactement telle quelle avant l'activation. Teste tout librement (objets garantis, badges visibles, aucune limite) — tout redeviendra exactement comme avant dès que tu désactives ce mode.",
    tester_mode_banner: "🧪 MODE TESTEUR ACTIF — rien de ce que tu fais ici n'affecte ta vraie progression",
    tester_mode_deactivate_confirm: "Ceci va effacer tout ce que tu as fait en mode testeur et restaurer exactement ta progression réelle d'avant. Continuer?",
    ...
    xp_source_argent_value: "+5 XP",
    xp_source_or: "Trophée Or du jour",
    xp_source_or_value: "+10 XP",
    ...
    xp_source_note: "Annuler une série retire aussi ton XP.",
    ...
    use_item_amulette_result: "Boost ×{mult} actif pendant {minutes} minutes!",
    use_item_don_xp_result: "+{xp} XP accordé instantanément!",
    use_item_graine_result: "+{xp} XP bonus sur tes habitudes d'aujourd'hui!",
    use_item_plume_result: "Ton objectif d'aujourd'hui est maintenant de {goal} push-ups.",
    ...
    undo_item_warning: "Annuler cette série va aussi retirer l'objet que tu viens de trouver. Continuer?",
  ...
  en: {
    ...
    tester_mode_title: "TESTER MODE",
    tester_mode_toggle_label: "Enable tester mode",
    tester_mode_warning: "Your progress is saved exactly as it is right now before turning this on. Test anything freely (guaranteed items, all badges visible, no limits) — everything goes back to exactly how it was the moment you turn this off.",
    tester_mode_banner: "🧪 TESTER MODE ACTIVE — nothing you do here affects your real progress",
    tester_mode_deactivate_confirm: "This will erase everything you did in tester mode and restore your exact real progress from before. Continue?",
    ...
    use_item_amulette_result: "×{mult} boost active for {minutes} minutes!",
    use_item_don_xp_result: "+{xp} XP granted instantly!",
    use_item_graine_result: "+{xp} bonus XP on today's habits!",
    use_item_plume_result: "Today's goal is now {goal} push-ups.",
    ...
    undo_item_warning: "Undoing this set will also remove the item you just found. Continue?",
    mythic_toggle_on: "On",
...
function applyTesterModeUI(active){
  $('#tester-mode-banner').hidden = !active;
  document.body.classList.toggle('tester-mode-active', !!active);
  const toggle = $('#tester-mode-toggle');
...
function playEchoDoreMelody(){
  ...
  const rhythm = [
    {d:0.00,f:523.25,g:0.09,dur:0.16},
    {d:0.18,f:523.25,g:0.09,dur:0.16},
    {d:0.36,f:659.25,g:0.10,dur:0.16},
    {d:0.54,f:523.25,g:0.09,dur:0.16},
    {d:0.72,f:659.25,g:0.10,dur:0.16},
    {d:0.90,f:783.99,g:0.11,dur:0.20},
  ];
  rhythm.forEach((n) => playTone(ctx, n.f, now + n.d, n.dur, n.g));
  const build = [659.25, 698.46, 783.99, 880.00, 987.77, 1046.50];
  build.forEach((f, i) => playTone(ctx, f, now + 1.20 + i * 0.13, 0.32, 0.08));
  const finaleStart = 2.05;
  const finale = [523.25, 659.25, 783.99, 1046.50];
  finale.forEach((f) => playTone(ctx, f, now + finaleStart, 1.5, 0.10));
  playTone(ctx, 1567.98, now + finaleStart + 0.03, 0.7, 0.06);
}
...
function startRainbowMode(){
  ...
  _rainbowInterval = setInterval(() => {
    hue = (hue + 1) % 360;
    applyAccent(hexFromHue(hue), true);
  }, 120);
}
...
async function loadToday(){
  ...
  updateActiveBoostDisplay(invData.activeBoosts);
  invData.mythicItems.forEach((it) => {
    if (it.cosmetic) applyMythicCosmetic(it.id, it.active);
  });

  const testerState = await api('/api/tester-mode');
  applyTesterModeUI(testerState.active);
...
async function loadInventory(){
  ...
  updateActiveBoostDisplay(data.activeBoosts);
}
...
function renderMythicCard(it){
  ...
  return `
    <div class="${cls.join(' ')}" ${(it.cosmetic && it.discovered) ? `data-toggle="1" data-mythic-id="${it.id}"` : ''}>
      <div class="inv-icon">${iconMeta.icon}</div>
      <div class="inv-name">${name}</div>
      ${toggle}
...
async function toggleMythic(itemId){
...
async function useItemDirect(itemId, instanceId){
  try {
...
async function updateActiveBoostDisplay(boosts){
  ...
  const row = $('#active-boost-row');
  if (_boostTimerHandle) clearInterval(_boostTimerHandle);
  if (boosts && boosts.amuletteEndsAt && new Date(boosts.amuletteEndsAt) > new Date()){
    row.hidden = false;
    $('#active-boost-text').textContent = t('active_boost_amulette', { mult: boosts.amuletteMult });
    const tick = () => {
      const remaining = new Date(boosts.amuletteEndsAt) - new Date();
      if (remaining <= 0){ row.hidden = true; clearInterval(_boostTimerHandle); return; }
      const m = Math.floor(remaining / 60000);
      const s = Math.floor((remaining % 60000) / 1000);
      $('#active-boost-timer').textContent = `${m}:${String(s).padStart(2,'0')}`;
    };
    tick();
...
function init(){
  ...
  $('#tester-mode-toggle').addEventListener('click', async () => {
    const current = await api('/api/tester-mode');
    if (current.active){
      if (!confirm(t('tester_mode_deactivate_confirm'))) return;
    }
    const res = await api('/api/tester-mode/toggle', { method: 'POST' });
    applyTesterModeUI(res.active);
    location.reload();
  });
