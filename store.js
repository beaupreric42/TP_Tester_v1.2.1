...
const ITEMS = [
  ...
  },
  {
    ...
    rarities: {
      basique: { mult: 2, minutes: 20 }, rare: { mult: 3, minutes: 25 },
      epique: { mult: 4, minutes: 30 }, legendaire: { mult: 5, minutes: 45 },
    },
  },
  {
...
function computeXP(db) {
  ...
  const xp = totalPushups * 1 + cleanCannabisDays * 20 + cleanCafeDays * 10 + marcheDays * 15
    + platinumWeeks * 50 + trophyXP + badgeXP + itemBonusXP;
  return { xp, totalPushups, cleanCannabisDays, cleanCafeDays, marcheDays, platinumWeeks, trophyXP, badgeXP, itemBonusXP };
}
